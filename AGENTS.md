...
- **The hazard is reentrancy, never concurrency.** This is a single-threaded STA application: one thread, no locks, no atomics, no `volatile`, and `IExplorerBrowser`/`IExplorerBrowserEvents` are STA-bound, so Windows messages and Shell callbacks are dispatched sequentially. Nothing observes a half-finished state from another thread, so do not write guards for that — no memory ordering, no atomicity for multi-step scalar updates, no defence against a sibling mutating a `PaneState`. What you must still handle is that a call which pumps our loop (a Shell call, `TrackPopupMenu`, `MessageBoxW`) dispatches the next message *nested* inside the current one: re-verify identity (`active()`, `pane_state()`, `is_shutting_down()`) after every pump point, and never hold a reference or pointer across one. A depth counter plus a deferral list (`ShellCallScope` / `shell_call_depth`) is the right tool for that; a lock is not. Two exceptions, both real: cross-*process* coordination for single-instance relay, and the reference count of a COM object handed to `shell32` (`src/com_ref_counted.h`), which third-party code releases at times we do not control.
