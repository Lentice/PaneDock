...
### 2026-08-28 隔離 worktree／bisect 調查：判定為驗證偽陽性，產品碼不需修改

- 重現前置：先對前一輪回報的 PID `35748` 執行不帶 `/F` 的 `taskkill /PID 35748`，系統回報 process 已不存在；另以 `Get-Process -Name PaneDock` 確認沒有其他 PaneDock instance，才開始新測試。live tree 的 PD-098 未提交差異（`src/app_shell/main.cpp` glyph/fallback 與 PD-098 交接區）全程未 staged、未改寫；調查在 `E:\GitHub\panedock-bisect` detached worktree 執行。
- 固定輸入：測試前 `%LOCALAPPDATA%\PaneDock\session.json` SHA-256 為 `73BDAC6E8CD14786846D0F4D0D043F87DC7AB3E0BDF8DA8EB689563AFB116F74`，`clean_shutdown=true`、55 Groups、active Group `default`、`four_pane_grid`、active pane `pane-0`；四個 active-tab locations 依序為 `C:\Program Files`、`C:\Windows`、`C:\Users`、`C:\Program Files`，均為本機 location。每輪啟動前快照 primary/backup，程序退出後還原；最終 primary hash 與上述值相同。各輪開始時均確認無其他 PaneDock process，沒有平行 build／launch。
- 依使用者更新方向先做 git history／bisect feedback loop，沒有加入 stderr probe。最初沿用原回報的單一 `FindWindowW` 判準時，current HEAD `66a6b5c` 與 PD-106 已知歷史點 `30ef6cd` 都被判成「30 秒無 HWND」；但 `30ef6cd --diagnostic` 同時輸出 `panedock.live_view_count=1 -> 4 -> 0`，直接證明 `WM_CREATE`、active-pane realize 與 deferred realize 已經執行，故該紅訊號不可信，不能拿來 bisect commit。
- 修正後的 throwaway harness 用 `System.Diagnostics.Process.Start` 立即取得 PID（不等待 GUI input-idle），每 250 ms 最長 30 秒交叉使用 `Process.MainWindowHandle`、`FindWindowW`、`EnumWindows + GetWindowThreadProcessId + GetClassNameW` 定位真實 `PaneDockMainWindow`，再以 `IsWindowVisible`、`GetClientRect`、`EnumChildWindows` 驗證可見且有非零 client geometry。這個 loop 同時記錄「HWND 首次建立」與「可見」兩個時間，避免把慢啟動誤報為從未建立；cleanup 對已定位的主視窗明確送 `WM_CLOSE`，所有最終驗證輪次 `forcedCleanup=False`。
- 端點結果：`30ef6cd` normal 為 `firstHwnd=692 ms / visible=1567 ms / 1920x967 / 52 children`；diagnostic 為 `735 / 2200 ms / 53 children`。current HEAD `66a6b5c` normal 連續三次分別為 `1239 / 3754 ms / 52 children`、`1161 / 3703 ms / 52 children`、`1596 / 6038 ms / 53 children`，3/3 均出現可見主視窗；diagnostic 為 `748 / 2178 ms / 53 children`。第三次 normal 的可見時間 `6038 ms` 已超過原始 `13 x 400 ms = 5.2 秒`判定窗，具體證明 5 秒級門檻不足以區分「顯著延遲」與「永不建立」。啟動約 2.2–6.0 秒，與 PD-106 graceful-close 驗收約 4.3–5.0 秒屬同一秒級量級，未觀察到無限等待。
- 最終判定採 Acceptance 1(b)：current HEAD 沒有可重現的 no-window 產品缺陷，也不存在可供 `git bisect` 的 bad endpoint；原始回報由過短的 5.2 秒觀察窗與不可靠的單一路徑 HWND harness 造成偽陽性。沒有 culprit commit、沒有產品碼修正，也沒有加入 timeout、retry、timer、background worker 或 probe。source inspection 仍確認 §9.3 邊界不變：startup pending 時 `apply_layout` 只允許 active pane 同步 initialize，`kDeferredRealizeMessage` 才以 `realize_deferred_panes=true` realize 其餘可見 pane。
- 下次同症狀的優先檢查順序：先排除殘留 PaneDock PID／single-instance mutex，再把「HWND 建立」與「視窗可見」分開計時至少 30 秒，至少使用 `Process.MainWindowHandle` 加 `EnumWindows` 交叉確認，不以單一 `FindWindowW` 或 5 秒逾時定案；保存 session hash、Group/location 摘要並做 normal／`--diagnostic` 對照。只有修正版 loop 出現真實 bad endpoint 才進一步 bisect 或加 `[PD109-PROBE]`。
- Agent checks（乾淨 current-HEAD worktree）：`cmake --build build-pd109` 成功；`ctest --test-dir build-pd109 --output-on-failure` 為 5/5 passed；`rg -n "\[PD109-PROBE\]|PD109-PROBE|startup.*timeout|background.*thread" src` 零命中；最終 `git diff --check` 通過。
