...
### 2026-08-28 根因更正、修正與最終驗收

- 本段以後續可重現證據更正上一段結論。上一輪 harness 在 `PaneDockMainWindow` 已建立但 `IsWindowVisible=False` 時即送出 `taskkill`，因此量到的是啟動競態；改為等待 `IsWindowVisible=True` 後，未修改產品碼的 baseline 精確重現原始症狀：PID 7556／39716 在第一次非強制 `taskkill /PID` 後 `EnumWindows` 已無任何該 PID 頂層 HWND，但 PID 超過 5 秒仍存活。這證明 tooltip 假陰性與真正的 main-window-gone 殘留是兩種不同模式，不能以前者推翻後者。
- 一次性 `[PD106-PROBE2]` stderr 階段計時在 PID 39716 釘死阻塞邊界：`WM_CLOSE enter` 後三個當輪 initialized `IExplorerBrowser::Destroy` 分別約 `63/62/47 ms`，`destroy_explorers leave live=0`，接著 `DestroyWindow` 返回並記錄 `WM_CLOSE leave`；但沒有出現 `message loop leave`。所以阻塞不在 `save_now`、`ExplorerHost::destroy`、`IExplorerBrowser::Destroy`、第二次 `destroy_explorers`、`OleUninitialize` 或 `CloseHandle`，而是 outer `GetMessageW` 沒收到可結束迴圈的 `WM_QUIT`。
- 根因是 `PostQuitMessage(0)` 原本在 `WM_DESTROY` 內執行；此時呼叫端的 `DestroyWindow(window)` 尚未返回，Shell child teardown／nested message work 仍可重入並消耗 queued `WM_QUIT`，導致外層 message loop 繼續等待。這也早於 §9.4「destroy 主視窗完成後，再退出訊息迴圈」要求的邊界。最終修正只把同一個 `PostQuitMessage(0)` 從 `WM_DESTROY` 移到 `WM_CLOSE` 中 `DestroyWindow(window)` 返回之後；保存 session、destroy 全部 live browser、destroy parent HWND、退出迴圈與 `OleUninitialize` 的順序完全不變，未新增 thread、hook、timer 或 cleanup short-circuit。
- 修正後 Release 最終驗收（每輪均先由 `EnumWindows` 確認 visible `PaneDockMainWindow`，由 `EnumChildWindows` 取得真實 child HWND/ID，再送一次不帶 `/F` 的 `taskkill /PID`）：diagnostic PID 45820／46424／3740 分別在 `4815/4973/4333 ms` 內 PID 與全部頂層 HWND 同時消失，連續 3/3 PASS；normal PID 1616 在 `4965 ms` 內同時消失，1/1 PASS。每輪結束後均無 PaneDock PID 殘留，未重送第二次訊號。
- diagnostic stdout 三輪都記錄 startup `panedock.live_view_count=1`，關閉後 `panedock.live_view_count=0`；probe baseline 亦證明當輪每個 initialized host 的 `Destroy` 都在 parent `DestroyWindow` 返回前完成。訊息迴圈後的既有防禦性 `destroy_explorers` 因 host 已 uninitialized 不會重複呼叫 browser `Destroy`。
- 最終 graceful close 後 `session.json` 實測 `clean_shutdown:true`，`session.json.bak` 存在且更新；測試前 primary 已備份，驗收後完整還原，還原檔與測試前備份 SHA-256 同為 `2148BCC06A86F760C4A85DA029AF596D77D0D76BF62EF2DE9760E28340EE938A`。所有 `[PD106-PROBE]`／`[PD106-PROBE2]` 暫時 instrumentation 已從產品碼移除。
