...
3. **固定版型優於任意分割。** 六種版型涵蓋實際需求,遞迴分割只增加模型與實作複雜度。
...
- 六種固定版型:單一、左右、上下、三分割、上方兩格併排下方一格全寬、四宮格
- 每個 pane 一個以上的 tab
...
在當前 Group 內可切換六種版型。pane 數量由版型決定。
...
支援且僅支援六種版型:單一、左右、上下、三分割、上方兩格併排下方一格全寬、四宮格。切換版型時,若新版型的 pane 數較少,超出的 pane 之 tab 依序併入保留的 pane;若較多,新增的 pane 以預設 location 開啟一個 tab。
